...
main = do
    ...
    let ngrammed = sortGrams $ aksaraGrams (optGrams as) split
    {- let reducedgrams = reducer (optFuzzy as) (optSingletons as) ps ngrammed -}
    let singles = optSingletons as
    let allgrams = allGrams ngrammed singles
    let reducedgrams = if optFuzzy as == True 
        then reduceGrams allgrams ps singles
        else map (\x -> [fst x]) allgrams
{-    putStrLn $  ',' :
        (intercalate "," $ 
            map (\xs -> quotate $ intercalate ";;" $ 
                map (transliterateString slp1' iast . gramsToString) xs) reducedgrams)
-}
    let aligned = alignGrams ngrammed reducedgrams
    if optFormat as == "csv"
        then mapM_ putStrLn $ printGrams aligned
        else mapM_ putStrLn $ formatNexus aligned reducedgrams
...
formatNexus taxa labels =
        [
            "#NEXUS",
            "BEGIN DATA;",
            "  DIMENSIONS NTAX=" ++  show (MM.size taxa) ++ " NCHAR=" ++ show (length labels) ++ ";",
            "  FORMAT",
            "    DATATYPE=STANDARD",
            "    GAP=-",
            "    MISSING=?",
...
{-        [
            "#NEXUS",
            "BEGIN TAXA;",
            "  DIMENSIONS NTAX=" ++  show (MM.size aligned) ++ ";",
            "  TAXLABELS " ++ (intercalate " " . map quotate' . MM.keys) aligned ++ ";",
            "END;",
            "",
            "BEGIN CHARACTERS;",
            "  DIMENSIONS",
            "    NCHAR=" ++ show (length reducedgrams) ++ ";",
            "  FORMAT",
            "    DATATYPE=STANDARD",
            "    GAP=-",
            "    MISSING=?",
            "    SYMBOLS=\"A B\";",
            "  CHARSTATELABELS",
            printNexLabels reducedgrams ++ ";",
            "MATRIX",
            intercalate "\n" (printNexGrams aligned) ++ ";",
            "END;"
            ]
-}
